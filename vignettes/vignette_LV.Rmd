...
smoothing_mean <- smoothing_exp %>% 
  ...
  rename(X1 = V1, X2 = V2)
ggplot(mapping = aes(x = X1, y = X2)) +
  geom_point(data = results, mapping = aes(alpha = weight), col = "blue") +
  geom_path(data = empirical_mean, col = "darkgreen", size = 2) +
  geom_path(data = smoothing_mean, col = "red", size = 1.5) +
  geom_path(data = as.data.frame(simulated_process)) 
```
