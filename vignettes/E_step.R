...
my_seed <- as.integer(readline(prompt = "Enter seed (0 means starting from true)"))
my_name <- readline(prompt = "Enter a name: ('only_a', or 'all')")
...
if(my_seed == 0){
  param0 <- true_param
  res <- EM_function(obs = observations, 
...
if(my_seed > 0){
  set.seed(my_seed)
  param0 <- generate_candidate_list(true_param, standard_dev_list = sd_list)
